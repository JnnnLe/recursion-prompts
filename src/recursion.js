...
var sum = (array, nextElement, acc) => {
  nextElement = nextElement || 0;
  acc = acc || 0;

  if (nextElement === array.length) return acc;
  return acc + array[nextElement] + sum(array, nextElement + 1, acc);
};
...
var sumBelow = n => {
	if (n === 0 || n === 1) return 0;
	if (n >= 2) return (n - 1) + sumBelow(n - 1);
...
var reverse = (string, i = 0)  => {
  ...
  return reverse(newStr.join(''), i + 1);
};
...
var modulo = function(x, y) {
	...
	//make a case for when both positive 
	if (x < y && x > 0 && y > 0) {
		console.log('what is x ', x) 
		return x;}

	//make case for when they are not negative
